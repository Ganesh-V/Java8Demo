...
public class Filtering2 {
	...
	public static void main(String[] args) {
		List<Student> properAgeStudents = filterMaleStudent(students, true);
		for(Student student:properAgeStudents) {
			System.out.println(student);
		}
	}
	
	public static List<Student> filterMaleStudent(List<Student> students, boolean isProperAge) {
		List<Student> result = new ArrayList<>();
		for (Student student : students) {
			if ((isProperAge && student.getAge() >= 0) ||
					!isProperAge && student.getGender() == Male) {
				result.add(student);
			}
		}
		return result;
